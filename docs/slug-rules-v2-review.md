...
## 第2ラウンド(同日・ユーザ裁定A/B/C反映済)

- **A 定着綴り14語追加**: koshien/oedo/shogun/shogi/judo/hokkaido/ryukyu/sumo/kendo/tokai/tohoku/kyudo(+既存tokyo/kyoto/osaka/kobe)。 ★カントウは「巻頭」(ドラえもん巻頭まんが作品集)と同音のため不採用。
- **B 人名はヘボン維持**: token単位の定着綴り化は同読み一般語を巻き込む(シュウイチ=週一/コウイチ=高1/ユウキ=勇気・結城)ため原理的に不安全 + AniList圏はTarou/Yuuki表記=長音保持の検索根拠と整合。
- **C 埋込カタカナ外来語の英語化**: kata-dict(2,412語)をガード付きでtoken適用(全カナ2字以上/値=英字3字以上/切断防止)。 乙女ゲーム→`otome-game`/東京喰種→`tokyo-ghoul`/植物モンスター娘→`monster-musume`。 junk(オ→wo/スローライフ→life)は排除。 悠々自適/悠優は正しくヘボン残存。
- **2b**: 幽☆遊☆白書=`yu-yu-hakusho`。 ★本編が2ページに分裂していた(ホーム社文庫名義 vs 週刊少年ジャンプ編集部名義=クレジット汚染)のを統合(公知: ホーム社=集英社G文庫版元+同題同作者)。 同型の hunter-hunter 等は適用後スイープで。
- **最終: 71,873ページ / 不正重複0 / 交差0 / junk0**。
