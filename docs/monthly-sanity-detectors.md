...
- 検出器 = `scripts/_audit-edition-lead-gap.py` / 出力 = `docs/production-diagnostics/edition-lead-gap.tsv`
- ★2026-09-07 ユーザ発見(「鉄腕アトムが漫画内分裂している」)。 版タブが**途中の巻番号から始まり**、
  同じ頁の別タブが下の巻を持つと、1本の作品が割れているように見える。
  実体は分裂ではなく **その版の先頭巻がまるごと落ちている**(充填案件)。
  鉄腕アトム= 版「講談社コミックス(1993)」が v6,8-15 から始まっていた。 機械証拠は楽天ローカル題索引:
  `4-06-1013xx` = KCスペシャル 全7巻 / `4-06-3133xx` = 講談社コミックス 全15巻 = **別ライン**。
  v1-5,7 は種2に在るのに canonical が上書きして消していた。
- 判定 = 版が①2冊以上 ②ISBNを1冊以上持つ ③最小巻>1 ④同じ頁の別タブが下の巻を持つ ⑤最小巻<=20。
  正当な途中開始(新装版が5巻から復刻等)も拾うが、充填器の5ゲート(題完全一致/版元prefix/ISBN連番/
  発売日/著者)で**実在する本が見つかった時だけ**埋まる自己限定型。
- 月次で見るもの = **版数の新規増加**。 増えたら `--emit-targets` → `_volgap-local-fill-v2.py --targets`
  → `_volgap-adjudicate.py` → `_volgap-apply-fill.py --apply` の充填チェーンに流す。
- 初回(2026-09-07) = 99版 / 95頁 / ターゲット294巻 → 69巻を適用(24頁)。

## 30. 漫画内分裂層 (刊行runが同一頁の版タブに割れる)

- 検出器 = `scripts/_audit-intra-page-run-split.py` / 出力 = `docs/production-diagnostics/intra-page-run-split.tsv`
- 既存2本の穴を埋める: `_audit-canonical-imprint-split.py` はレーベル名の一致で探すので名前が違うと取れず、
  `_audit-edition-run-split.py` は**巻番号の重複ゼロ**を要求するので1巻でも重なると取れない。
- 判定 = 同一頁の2版が ①出版社/ISBN出版者記号が共通 ②合わせると連番 ③巻ごと最古の刷で日付が単調
  ④★**重なりが小さい**(重複<=2冊 かつ 重複/合併<=20%) ⑤どちらも単独では完結していない。
  ④が核心 = AKIRA(1984年版[1-6] と 2003新装版[1-6] = 完全重複)のような**正当な別版**を弾く。
  別版は全巻を丸ごともう一度出すので重なりが大きく、runの分裂は前半/後半なので重なりが小さい。
- 月次で見るもの = tier A/B の新規増加。 ★**自動統合は禁止**([[imprint_split_arms_type]]
  [[edition_run_split_arms_wide_type]])。 楽天seriesName・ISBNブロック・刊行リストで1件ずつ裏取りする。
  ★鉄腕アトムのように「統合ではなく充填が正解」の場合がある = 先に #29 を疑う。
- 初回(2026-09-07) = 8ペア / 8頁(全て tier B)。

## 31. 同人セレクション層 (ハル/POE BACKS型)

- 検出器 = `scripts/_audit-doujin-selection.py` / 出力 = `docs/production-diagnostics/doujin-selection.tsv`
- ★2026-09-07 ユーザ発見(頁「ハル」= hal-ayase)。 唯一の巻 9784865893540 の楽天 seriesName が
  **「POE BACKS 同人作家コレクション 268」** = 同人誌の商業セレクション。
  CLAUDE.md の掲載対象外(drop imprint patterns の `同人`)に該当するのに載っていた。
- ★**レーベル名では判別できない**: imprint は「Poe backs」で、同じレーベルには
  商業BL(Baby comics / Bébé comics / ザオメガバースプロジェクトコミックス)が31頁居る。
  判別できるのは**楽天の seriesName** だけ。
- 判定 = 本番掲載ISBNのうち楽天 seriesName/title/subTitle に「同人」を含むもの。
  **SERIES一致 = 強い信号**(seriesName に出た) / **TITLE一致 = 弱い信号**で分ける。
  ★TITLE一致は「同人を題材にした商業漫画」(大同人物語 / 美少女同人作家と若頭 / 同人はっけん伝 /
  電波教師 / 阿佐ヶ谷サキュバス同人物語)なので**掲載対象**。 混同しない。
- 月次で見るもの = **SERIES一致の新規増加**。 出たら実体を確認して non-manga-drop.yml へ。
- 初回(2026-09-07) = SERIES 14巻/6頁 → 全6頁を drop
  (同人作家コレクション / ハル / いちこ / 木野芽ササ / ヤタ = POE BACKS 同人作家コレクション。
   ティータイム = 白夜書房「同人誌まんが傑作選」全7巻・多数作家のアンソロジー)。
  ★POE BACKS 同人作家コレクションは **題 = 作者名**(作家個人の同人セレクション)という特徴がある。
