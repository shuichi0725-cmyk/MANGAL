...
## 内包副題278の著者+巻構造 精選(2026-06-03・慎重)

★決定的発見: 278全ペアが ★**同一anilist_idを共有**(SEP_DIFF_AID=0)。 = ★**spinoff(Fate藤丸立香/86オペレーション・ハイスクール/ロードス英雄騎士伝)も本編と同じ著者+aid+独自巻番号を持ち、 著者・巻・aidのどれでも本編arcと機械的に区別できない**(ゲゲゲ「franchiseを1idに束ねる」の pair版)。

- 巻連続/重複だけで merge すると spinoff を誤統合 → sub≤4 でも続編(ステラ/蟲姫2/空の軌跡SC/JR)が漏れる事を検出 → ★**104件適用を即revert**。
- ★最終 = ★**純粋な版/フォーマットマーカー(perfect/DX/remix/漫画/deluxe/プロローグ/完結編)の9件のみ merge**。 残269は ★**安全デフォルト分離**(実質副題=per-case Wikipedia以外に確実な手段なし)。

★教訓: ×2内包副題層は機械分類の限界。 確実なのは「版マーカー」のみ。 spinoff vs arc は per-case知識(Wikipedia)が必須で、 安易な構造mergeは本番DBに spinoff混入を招く。

## 内包副題269の個別Wikipedia確証(2026-06-04・慎重)

巻数(人気度)順に個別判定。 Wikipedia確証 + 構造証拠 + title判定。 applier=`_apply-x2-decisions.py`(match/merge/drop/separate)。

### Wikipedia確証の主な判定
- **MERGE(同一作)**: かりあげクン(=ほんにゃらゴッコ改題)/ 太陽の黙示録(群雄編=第1部)/ ナッちゃん(東京編=連続24巻)/ Jドリーム(3部=同一連載)/ 相棒(=たった二人の特命係 全題)/ アタゴオル(同一シリーズ)/ ブレイド三国志赤壁(追加エピソード)/ 百花繚乱サムライガールズ 他
- **DROP(関連書/傑作選/画集)**: こち亀下町奮戦記(傑作選)/ 深夜食堂の勝手口(グルメ本)/ 賭ケグルイ愛(ファンブック)/ 哲也逆転のセオリー(ガイド)/ まじかるタルるート魔法アイテム(設定資料)/ 魔法陣グルグルランド(関連書)/ あまつき絵巻金華糖(イラスト集)/ 神様ドォルズ隻・詩緒(傑作選)/ ひみつのきんいろモザイク(画集)
- **SEPARATE(別作/spinoff/続編)**: 夢幻紳士各篇(別世界)/ 北斗の拳イチゴ味(パロディ)/ ねこねこ日本史ヒーロー列伝(spinoff)/ 賢者の弟子マリアナ(spinoff)/ とでんか少年探偵団(spinoff)/ サトラレneo・へろへろくん特上・なんて素敵ジャパネスク人妻編・悪魔の花嫁最終章(続編)/ 超人ロッククアドラ・夢幻紳士(独立エピソード)

### title判定で確定(高確度=titleが証拠)
- 副題/全題/グロスmerge: 聖伝RG VEDA / MOON昴 / マホロミ時空建築幻視譚 / イルゲネス偽翼の交響曲 / 薬師寺涼子霧の訪問者 / GO DA GUN〔銃〕 / 呪怨juon / 轟世剣ダイ・ソード / オールド・ボーイ(ルーズ戦記) 等 多数
- spinoff/続編marker分離: ステラ/JR/Ⅱ/neo/MAX/外伝/4コマ/絵本 等

### 累計(×2ペア 677全体)
- ★**MERGE = 約250件**(版/fragments/表記揺れ/グロス/Wikipedia確証の副題・改題・連続作)
- ★**DROP = 約30件**(アニメ/外国版/画集/設定資料/ファンブック/傑作選/関連書)
- ★**SEPARATE = 残**(spinoff/続編/パロディ/別漫画化 + 低巻数の obscure tail=安全デフォルト分離)

★誤merge回避を最優先(@note YAML破損も検出修正)。 spinoff/続編はジョジョ型方針で別ページ、 確証取れた同一作のみ統合。
