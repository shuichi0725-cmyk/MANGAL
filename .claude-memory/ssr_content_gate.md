...
## 床はクラス別 = ページ送り末尾頁(2026-09-22 追加)

`/publisher/asahi-shimbun/2` が「本文140字 < 床150」で FAIL したが、**実HTMLを開いたら正常**だった
(301作品の2頁目に掲載1作品が書影・著者・年つきでサーバー描画済み)。= 偽陽性。

★**床は定数を動かさず、クラスを切り出して実測から決める**(このscript自身の作法)。実測 2026-09-22:
- 対象 416頁(`magazine` 36 / `publisher` 178 / `year` 202 の `<key>/<n>`)
- 頁固有文字数 **最小140字 / 中央12,004字**。150未満は1頁のみ、次点153字(どちらも掲載1作品)
- ★**416頁すべてに `/manga/` リンクが1本以上**在る = 空描画なら作品リンクごと消えるので、
  「1作品しか載らない正当な末尾頁」と「client専用描画」は本文長で十分に分かれる

→ `RE_PAGINATED = ^(?:magazine|publisher|year)/[^/]+/\d+\.html$` に床 **60**(140字に対し余裕2.3倍)。
負テスト済み: `publisher/x.html`=150 / `tokushu/…/1.html`=150 / `manga/x.html`=150 = **コーナー頁と作品頁の床は不変**
(2026-09-18 の空コーナー事故に対する検出力は落ちていない)。

★偽陽性を放置すると「毎週鳴るから無視する」になり番人が死ぬ。偽陽性は**握り潰さずクラス化**して消す。
