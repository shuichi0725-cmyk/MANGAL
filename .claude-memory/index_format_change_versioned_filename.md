...
- ★**索引ファイルを新しく足す時**(2026-09-23 列形式索引で確立): 配信する一覧は `scripts/_index_files.py` の `INDEX_FILES` が単一ソース(r2-sync/差分反映/機能蒸留の除外/週次finalize purge が共有)= そこに1行 + `_audit-index-hygiene.py --accept-format` で契約登録(★その際 head の fl が `?` に揺れて契約が弱まるので、差分を見て既存行は元の型に戻す)+ robots の Disallow + preview workflow の cp。
