...
★**ISBNが無い記事でも「単行本」節は強い**(2026-09-06 ユーザ提示のロック冒険記で実証)。
版ごとの **「全何巻」** が列挙されているので、**ISBN以前(〜1980)の作品では全巻数の唯一の根拠**になる。
これで判ること= ①どの版が初出か(=通常版タブに据えるべき版。全集が主版を占める型の是正) ②その版の巻数と
当方の保有数の差=**真の欠落か否か**(ロック冒険記=鈴木出版全3巻に対し当方2巻→2巻は本当に無い) ③種2で
レーベル表記が割れている版の**正しい巻数**(コダマプレス版=全2巻→3つ目の表記行は重複記録と確定)
④種2にあるのに頁へ出ていない版の実在確認(虫コミックス全1巻→版タブへ復活)。
★ただし日付は書かれていないことが多い。**日付を推測で埋めない**(NDLに近い日付の記録があっても、
巻の同定ができなければ入れない)。
