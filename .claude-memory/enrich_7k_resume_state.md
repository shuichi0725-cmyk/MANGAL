...
description: "キャッチ/詳細エンリッチの進捗と再開点。2026-07-30時点: full系バッチは全消化済。★現在の作業=短キャッチrequeue(batch-0001〜0190)の消化で、0174まで=3,698作を本番反映済/残376作・次はbatch-0175から。genre系0218-0380も実質完了(残121はmaster32該当なし)"
metadata: 
  node_type: memory
  type: project
...
## ★現在の作業 = 短キャッチ requeue の消化(次は batch-0175 から)
...
- **2026-07-31(5): batch-0139〜0148 = 223作を追加消化**。★**catchは「本文＋末尾フレーズ」の合成で書くと字数が安定する**:
  生成scriptに `def T(c,t): return c.rstrip('。')+'。'+t+'。'` を置き、`T("[フック。状況]","[ジャンル/締めの短句]")` で組む。
  素で書くと必ず42〜47字に落ちるが、この形なら50〜60字に乗る(0143/0144で違反0、0145以降も1〜2件のみ)。
- **2026-07-31(6): batch-0149〜0156 = 185作を追加消化**。★**「同じ長さの言い換え」で直そうとして空振りする**罠:
  48字未満を直すとき語を入れ替えるだけだと字数が変わらず、同じ違反が2〜3往復続く。
  **直すときは「必ず文字を足す」**(「〜物語」→「〜物語である」/「電光」→「電光石火」/主語や修飾を1語追加)。
- **2026-07-31(7): batch-0157〜0166 = 230作を追加消化**。★**catchは「3節構成」で組むと字数違反がほぼ消える**:
  `def T(a,b,c): return a+'。'+b+'。'+c+'。'` で `T("[フック]","[状況]","[ジャンル/締め]")`。2節のT()では42〜47字に落ちるが、3節なら50〜60字に安定して乗る。
  代わりに synopsis が76〜77字に落ちる回が出るので **synは82〜90字を狙う**。
- **2026-07-31(8): batch-0167〜0174 = 179作を追加消化**。★3節構成に切り替えて以降の残る詰まりは2種類だけ:
  (1) **`catch==syn頭20字` VIOLATION** = catchの第1節とsynの書き出しが同じだと踏む。**catchはフック・synは設定から**、と書き出しを必ずずらす。
  (2) **synが76〜77字で落ちる** = 3節catchに字数を持っていかれる分、synが痩せる。**synは82〜90字**を狙い、足りなければ末尾に一文(「〜も描かれる」「〜も登場する」)を足す。
  ★字数直しは**必ず文字を足す**(同義語に置換すると長さが変わらず同じ違反を2〜3往復繰り返す)。
- **残 = 376作(batch-0175〜0190)。次は batch-0175 から**。
