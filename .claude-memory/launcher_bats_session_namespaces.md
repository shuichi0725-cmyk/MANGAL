...
- **fable.bat** = メイン作業用 (fable[1m])。**opus.bat** = Opus 5.5/1M 併用(`claude-opus-5-5[1m]`・2026-09-23 commit 8294351ed)。**sonnet.bat** = 別窓start(モバイルRC・アイドル運転用、sonnet)。
...
- ★**エフォート(effort)は3本とも指定しない = CLI の既定のまま**(2026-09-23 ユーザ裁定「ディフォルトで変えないまま」)。モデル切替で「高→中」に見えた件の答え。固定したくなったら CLI 引数 `--effort <low|medium|high|xhigh|max>` を bat に足すだけ(ユーザ判断)。
