...
## ★2026-09-09 再発(別機構・同じ誤診)= [[rsc_txt_browser_cache_stale_navigation]]

同じ「直したのに直っていない」を**もう一度**踏んだ。今回の犯人は RSC ペイロード `.txt` が
`ASSET`(ブラウザ24時間)に落ちていたこと。**鉄則1を守らなかったのは私(モデル)側**で、
端末が新しいコードを受け取っているかを確かめる前に3つの仮説を出して全部外した。

★**具体化した鉄則1 = 最初に「シークレットタブで開いてもらう」**。
キャッシュ無しで正常なら配信は白・端末が黒、が10秒で確定する。散文の「確かめる」では守れなかったので、
**やることを1つの操作に落として書く**。併せて配信の健全性は**本番 vs 手元 out/ の md5 突合**で見る。
