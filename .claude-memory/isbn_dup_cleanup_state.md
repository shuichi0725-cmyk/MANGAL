...
metadata: 
  ...
  modified: 2026-07-28T16:31:45.242Z
---
...
**済(R6-R7 2026-07-07)**:
- R6 = ★**種4 qid注入バグ根治**: 種4エントリの`qid`は種2 qid=**作者QID**なので、qid付きエントリが作者の全作品ページに巻を注入(うしおととらワイド版2-9が藤田和日郎14作へ)。該当8エントリqid無効化+**promoteに恒久ガード**(qid一致sidは題合致時のみ採用)。91ペア消滅
- R7 = **本籍sid遮断**42block: R5遮断のすり抜け=スピンオフ題が本編題を含むサブ断片sid(西風の旅団⊃ログホラ等)がsubstring判定を通過して滲む穴を、本籍sid題と頁題の**完全一致**帰属で封鎖

**残(R7後: ダブリISBN 295個/ペア96件=DUP_PAGE54+SHARED_FEW42) = 最後の難所per-case**:
- **両頁同題(本籍題が両頁と一致し機械判定不能)~34組**、2型に分かれる:
  ①同一作の断片(コナン特別編=作画交代1シリーズ/復刻版マガジン大全集/NHKその時歴史が動いた等)→**片方set⊇他方の時だけ**page-dedup可(部分重なりでdropすると固有巻消失に注意)
  ②別作家コミカライズ同題(X-MEN3頁/VALKYRIE PROFILE4頁/リング/ウルトラセブン/ファイアーエムブレム/JOKER/魔法科2種等)→ISBNごとに**楽天delta author**で帰属判定→block+volume-exclude
...
**済(R9 2026-07-29)**: ★「blockが効かない」の根因解明=**carrier未遮断**。共有本は第三のクラスタ(carrier、例: ゾクこわい本122396)が運び、頁mainとのペアblockだけでは切れない(blockはmain×候補の対でしか効かない=promote L1949)。
- **R9=carrier遮断**: 残ペアの共有ISBN→種2本籍sid(carrier)を逆引き→**題が正規化不一致のmainからblock**(R7本籍遮断と同原理・題一致=自分の表記揺れ断片は遮断しない)。72block追加→**145→62ペア**(開始297から79%減)
- 検証=巻数before/after全数: 0巻化なし。こわい本各編10→1冊/本気!破門編75→6/バキ最凶死刑囚編31→7/弱虫逃避行編30→3等=**R8のflip9組(編頁worklist)が自然解決**。特攻の拓も固有3巻だけ残り解消
- **残62ペアの正体(真の最終層)**: ①両クラスタが同じISBNのコピーを持つ型(狼の口8→4=半分だけ解消・残りはdedup裁定要) ②別著者同題(ザ・コンバット等=R8 review組) ③src _skey無しでsid解決不可7stem(9番目のムサシ/人魚シリーズ/手塚シナリオ集等) — 全部per-case
続きのトリガー=「ISBNダブリの続き」。
