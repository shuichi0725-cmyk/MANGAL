...
## 少年サンデーコミックスの刷タブと書影(2026-09-22 ユーザ裁定)

`少年サンデーコミックス` は刷タブ2種を持つ:
- `初版カバー(1980-87)` 34巻 = ★**楽天に画像が無い。書影は元から存在しない**(ユーザ「通常版はもともとない」)。
  楽天に商品(題・著者・1980-03-22・394円)は在るが `書影無`。欠落ではないので埋めようとしない。
- `新装版カバー(2006-08)` 34巻 = 全巻に書影あり。一覧/検索のサムネイルもここから採られる。

他版は ワイド版15 / 文庫版18 / 復刻BOX(2022-23)4 で全巻書影あり(2026-09-22 に充填 → [[cover_null_despite_seed]])。
★**裁定済=埋めない**(ユーザ「初版カバーは違うものなので埋めないと前に決めた」。2026-09-22 に再確認)。
理由 = 刷タブ `初版カバー(1980-87)` は**その刷の装丁そのもの**を指す枠なので、
新装版カバーやKobo電子の書影を入れると**タブの意味が壊れる**(別の絵を「初版カバー」と名乗ることになる)。
書影ゼロのまま置くのが正。[[kobo_cover_wrong_for_old_print]] の「注意書きを添えて出す」は
**刷タブには適用しない**(あちらは版に1枚も書影が無い時の救済)。
