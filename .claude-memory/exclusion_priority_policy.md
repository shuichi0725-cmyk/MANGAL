...
## 追記 2026-09-24: リイド社「SPコミックス ポケットワイド」= コンビニ廉価(540〜679円)
- 予約頁の経路(日次蒸留)をすり抜けて3頁公開されていた(剣客商売プレミアム 家/包丁人味平(5)/女忍隠密剣 殲滅ス!!)→ drop+preorder-deny。
- ★予約ドラフトの生成ゲートに imprint「ポケットワイド」の拒否を足すかは未決(同社 SPコミックス 本体は一般=imprint一律は不可、ポケットワイドは狭いlabel)。
- ★**訂正(同日)**: ポケットワイドは**再録とオリジナル両方**の判型。純ポケットワイド頁73頁を楽天照会=約半数は他レーベルに同題同作者なし(大江戸酒日和/ウクライナ混成旅団/陸海軍航空隊戦記 等=リイド社誌連載の初単行本=オリジナル)。→ imprint一律dropは不可(7月の秋田トップW/Gコミックスと同じ結論)。
  予約分類(_preorder-classify.py)は09-14からポケットワイドを一律skip=オリジナル新刊も落ちる。ユーザに 1)一律維持 2)保留裁定に回す 3)既存73頁を1頁ずつ裁定 を提示中(推奨2+3)。判定材料 = scratchpad pw_check(同題同作者の他レーベル有無)だが、SPコミックス表記は同一本のレーベル表記違いのことが多く再録証拠にならない=ISBN一致を除いて判定し直すこと。
