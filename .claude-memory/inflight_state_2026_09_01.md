...
- ホーム/ハブ見直し4〜6(同日・コードのみ): ①フッターに「コーナー」行(ランキング/日替わり特集/愛蔵版/特装版/カラー版)+「ジャンル別」
  = 孤立していた5面に全頁から入口 ②ホームのJS描画コーナー6つにSSR初期値/枠(配信HTMLに見出し+一覧リンク・レイアウトずれ防止)
  ③/genre 目次頁を新設(ホームのタイル・ルーレット・ジャンル面上部から結線、sitemap固定頁に追加)。
  ★フッターは作品頁66kにも載る=作品頁ぶんは**週次でしか出ない**。ホーム・/genre は機能蒸留でも出る。sitemapの /genre は週次。

## 追記 2026-09-24(夜): データ変更も本番待ち(★すべて週次蒸留で出る。manga.v2/索引は反映済)
- ジャンル付与: 異世界/グルメ(タグ+明記)817頁 → 方法2〜4(専門レーベル/Wikipedia/紹介文)3,359頁。bl 2,030→3,293・4-koma 528→1,405・gag 757→1,672・samurai 139→303。除去0。
- ちいかわ一式: ジャンル+action/fantasy・分野seinen・開始2020・7/8巻日付・特装版5冊・掲載誌 x-twitter(新設master)・imprint ワイドKC・2/4巻書影(本文サンプル→表紙)。
