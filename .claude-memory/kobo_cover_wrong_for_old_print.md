...
## ★適用外 = 刷(カバー違い)タブ(2026-09-22 ユーザ)

「注意書きを添えて出す」は**版に1枚も書影が無い時の救済**。
`初版カバー(1980-87)` のような**刷タブは装丁そのものを指す枠**なので、別装丁の書影を入れると
タブの意味が壊れる。ここは**空のままが正**(うる星やつら= ユーザ裁定「違うものなので埋めない」)。
→ [[urusei_version_display_rules]]
