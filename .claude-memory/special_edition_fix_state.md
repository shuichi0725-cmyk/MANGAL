...
## 2026-07-03 特装機構の復活+重大バグ2種の根治
- ★旧修正=ページ直接パッチはフルpromoteで消えた→**promote最終passに恒久結線**(special seed 6ファイル/約1,960件・standard限定・通常差替+variants併存)+**normal主枠へのvariant添付拡張**(落語心中/ろこどる型)。
- ★★**出版社コード跨ぎ誤ペア47件を発見・全除去**(臨場コンビニ→王様ゲーム/秋田BJ→講談社全集/鬼の花嫁→人喰い鬼の花嫁等=同月+価格ヒューリスティック誤爆や「特定頁文脈の補正」)。グローバル適用で12頁が実際に汚染(別作ISBNに差替)→復元済。black-jack等は未再生成で無事=時限爆弾を先に解除。**promoteに sp[4:6]!=ni[4:6] 無視ガードを二重実装**。
- 新刊特装は自動で捕まらない(ベルセルク43/ちょびっツPREMIUM型)→月次サニティに特装検出を組み込む(検出器は_special_edition_fix_gen系)。
