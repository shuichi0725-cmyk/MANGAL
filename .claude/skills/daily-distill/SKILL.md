...
3. 新規掲載可が有れば: `.cache/backward/<年>/ai-todo.jsonl` の該当 TODO を記入
   - `is_manga`(非漫画なら false) / `slug`(規則: ヘボン式・ハイフン区切り・勝手命名禁止=NDL/楽天ヨミ基点) / `genres`(closed vocab) / `demographic`(shounen/shoujo/seinen/josei/kids/general) / `catch` / `synopsis`(60-120字・ネタバレ無し)
