...
| # | やること | ツール(コマンド) |
|---|---|---|
| 1 | 予約harvest | `python scripts/_rakuten-preorder-harvest.py` ← ★**最後の行を読む**: 「★★ 取りこぼし警告」が出たら進めない(楽天は1クエリ最大100頁=3,000件。頁上限で打ち切られると当月〜近い未来の新刊が**毎回同じ所で黙って落ちる**)。★上限にかかったジャンルは**自動で細分して取り直す**(子ジャンル→判型→在庫状況・2026-09-24〜。分けた先の総数合計が親に届くかを毎回検算し、届かなければ次の分け方でも取る)ので、★★が出るのは細分しても取り切れない時だけ=分け方(出版社別など)の追加が要る。「自動細分」行が出ても★★が無ければ進めてよい。「★ 取りこぼし注意」=在庫状況で分けた(品切れ本は在庫状況が空でどの値でも引けない=当月発売済みの品切れ本が漏れうるが、発売前の日次で取れていれば実害なし)=対応不要。「(情報)上限まで余裕が少ない」も対応不要。状態= `.cache/preorders/harvest-status.json`。2026-09-24 実測=その他 71/100頁 |
...
### ★2026-09-24 の恒久修正(巻表記5型・3か所すべてに焼いた)
楽天題の **巻ノN/巻之N/其ノN(漢数字可)・(第N集)/(N集)/第N集・(v.N)・(N(副題))** が読めず vol=None
= 既刊作品の続巻が**新作1巻として別頁化**されうる穴(当世幻想博物誌（巻ノ1）で発覚。充填側 parse_vol と同時是正)。
- ① `split_title` の **B0m**(matched=`marker_ext`)② `_preorder-gen-midfill.py` の `VOLP`(漢数字は `_kanji_int`)
  ③ `_preorder_draft_lib` の `_VOL_TAIL` / `_VOL_KANA`(ヨミ実測= 巻之五→「マキノゴ」「カンノサン」。★「ソノN」は剥がさない=題側の「その六」と揃わない)
- 検算の型: 楽天ローカル種37.3万件で新旧を突合 → 旧で決まっていた結果の変化は**7件(全部 suspect→確定 か 部分剥がし「巻の」→全剥がしの改善)**、
  新たに読めた 2,814(split)/2,822(VOLP)/2,820(_VOL_TAIL)/27(_VOL_KANA)。★規則を足す時は必ずこの突合を回す。
- 過去の巻き込み = **0件**(本番題・副題・予約保留に該当なし。題に表記を含む本番頁は『其の七の者』=本物の題だけ)。
...
- Layer1: 楽天booksGenreIdがdemographic裏取り(**9桁で引く**: 001001001=少年/001001002=少女/001001003=青年/001001004=レディース/001001012=その他(=決まらない)/001021…=BL)。★`001001`(6桁)は**漫画全体**=先頭6桁で引くと全部少年になる(2026-09-24 refresh-held で29頁実害)。
