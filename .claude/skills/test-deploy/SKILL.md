...
- ★**いまのセットはここに書かない**(頻繁に入れ替わり、書くと腐る。2026-09-23 に「3,000頁」の記述が残ったまま実体は178頁だった)。
  確認 = `ls .preview-data/manga | wc -l` + `git log -3 --format="%ci %s" -- .preview-data/manga`(入替コミットに選定理由を書く)。
  入替の流儀: ユーザが「〜だけ入れて」と**セットを名指し**した時は入替(9/21「穴1冊だけ」・9/23「1巻が無い」167頁)。
- 参考: 2026-09-08 の 3,000頁セット(200 → 3,000)の選定法 = 公開索引を
  `title_kana` の50音順に並べ、既存頁を残したまま**等間隔で充填**(人気順ではない=ジャンル/年代/分野/50音が偏らない)。
  改名頁は `.cache/prod-page-slugs.json` で 公開slug→SRC stem に解決する(これを忘れると実体なしで落ちる)。
  分布: アニメ化373 / completed 2,643・ongoing 353 / 分野6種すべて非0 / 1934〜2027年
  = **カテゴリタイル8枚が全部埋まる**ので、フィルタ・検索・件数表示を実寸に近い形で試せる。
