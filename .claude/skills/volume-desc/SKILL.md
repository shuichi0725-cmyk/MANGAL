...
## ★対象巻の選定 (= 2026-07-22 ユーザ裁定。scriptのtarget_volumes()に実装済=自動)

- **対象 = 主版(原則「一番最初に出た通常版」)の巻だけ**。選定はscriptが自動
  (type=standardのうち最古発売の版。standard皆無なら全版から最古)。
- **他版の巻(文庫合本・新装・全集・別巻割)は生成対象にしないし巻数も考慮しない**。
- **同じ巻番号がどこかの刷(versions)/版で説明済みなら、その巻は完了扱い**(二重生成禁止。
  うる星型=新装カバー刷ISBNに34巻分既存→初版カバー分を作り直さない)。
- **主版巻にcaptionが無く他版/他刷の同巻番号にある場合のみ**、そのcaptionを материалに書く
  (materials.jsonlの`caption_src`が代替源ISBN。説明の帰属=主版ISBNのまま)。
