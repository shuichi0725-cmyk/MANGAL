...
- ★**preview が 0件になったら必ずそう言う**(2026-09-15 実害: 116件を本番化して preview が空になり、
  ユーザが別件のUI変更を確認しようとして『テスト環境漫画なにもないけど？』となった)。
  残 K件=0 のときは「**テスト環境は空になりました。確認したい頁がある時は『テスト環境に出して』**」と添える。
