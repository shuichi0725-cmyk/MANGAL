...
type Forms = { q: string; qKana: string; qRoma: string };
...
function tokenForms(token: string): Forms {
  ...
  const qRoma = collapseVowels(normalizeForSearch(kanaToRomaji(token))); // かな入力→ローマ字(母音圧縮)
  return { q, qKana, qRoma };
}

/** 1作品×1語の照合(題名系・著者を併走)。 */
function rowMatches(hay: Hay, i: number, f: Forms): boolean {
  return Boolean(
    hay.title[i].includes(f.q) ||
      (f.qKana && f.qKana !== f.q && hay.title[i].includes(f.qKana)) ||
      (f.qRoma && hay.kanaRoma[i].includes(f.qRoma)) ||
      hay.au[i].includes(f.q) ||
      (f.qKana && f.qKana !== f.q && hay.au[i].includes(f.qKana)) ||
      (f.qRoma && hay.au[i].includes(f.qRoma)),
  );
}
...
export function searchSlugs(query: string, items: MangaListItem[]): Set<string> {
  ...
  for (let s = 0; s < scanLen; s++) {
    ...
    if (forms.every((f) => rowMatches(hay, i, f))) hits.push(i);
  }
