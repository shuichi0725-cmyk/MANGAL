...
function displayBlocks(editions: Edition[]): Edition[] {
  ...
  //   ★2026-09-01 追補(ユーザ報告=こち亀): 「全巻に書影」を **完全一致** で判定していたため、
  //   203巻中202巻に書影がある通常版が、未発売の最新1巻だけ書影が無いせいで tier1 に落ち、
  //   26巻フル書影の文庫版に先頭を奪われていた(北斗の拳/ハッピー!/エリア88 等 37頁で同型)。
  //   → **9割以上に書影がある版は「揃っている」扱い**にする(連載中の近刊が欠けても本編が先頭)。
  //   閾値方式にしたのは、静的書き出しで new Date() が使えない(=「未発売」を日付で判定できない)ため。
  const COVER_FULL_RATIO = 0.9;
  const coverTier = (b: Edition) => {
    const n = b.volumes.length;
    if (!n) return 0;
    const c = b.volumes.filter((v) => v.cover_url).length;
    return c === n || c / n >= COVER_FULL_RATIO ? 2 : c > 0 ? 1 : 0;
  };
