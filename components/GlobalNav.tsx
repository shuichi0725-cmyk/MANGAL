...
const NAV_SVG: Record<string, { d: string; circle?: [number, number, number] }> = {
  ...
  // 新刊=今月の新刊一覧(/shinkan)。≡メニューの「今月の新刊一覧」タイル(box)と同じ絵柄
  // ★2026-09-14 ユーザ指示: ラベルを「新作」→「新刊」に変更(行き先・絵柄は不変)。
  //   このキーは RIGHT_FIXED のラベルで引かれる= ラベルを変えるならここも必ず同時に変える
  //   (NAV_SVG に無いラベルは NavSvg が null を返し、**アイコンだけ黙って消える**)。
  新刊: { d: "M21 8l-9-5-9 5v8l9 5 9-5zM3 8l9 5 9-5M12 13v8" },
...
// ★2026-09-02 ユーザ裁定: 「一覧」を外し、検索を先頭に・その右に「新刊」(=今月の新刊一覧)。
//   ★2026-09-14 ユーザ指示でラベルを「新作」→「新刊」へ改称(行き先 /shinkan は不変)。
