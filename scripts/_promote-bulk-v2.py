...
def main():
    ...
    for ypath in _src_files:
        ...
        # ★同名タブの回避 (2026-08-29 ユーザ報告 orenosora型): 1頁に **まったく同じ label の版**が
        #   2つ以上並ぶことがある(同じレーベルで刷を重ねた別run / 種2のクラスタ分裂の残骸)。
        #   読者にはどちらがどちらか判別できないので、**表示名だけ**に初刊年を足して区別する。
        #   ★レーベル名(imprint)は事実なのでそのまま。巻・ISBN・発売日にも触れない。
        #   統合はしない(同名でもISBNが別=別の刷/別版のことがあるため。自動統合禁止)。
        _eds = new_yml.get("editions") or []
        _bylabel = {}
        for _ce in _eds:
            _bylabel.setdefault(str(_ce.get("label") or ""), []).append(_ce)
        for _lb, _grp in _bylabel.items():
            if len(_grp) < 2 or not _lb:
                continue
            # 既に末尾に (…) が付いている同名同士なら、その括弧を外して付け直す
            _base = re.sub(r"\([^()]*\)$", "", _lb).strip() or _lb
            def _first_date(e):
                _ds = [str(v.get("release_date") or "")
                       for vs in [e.get("volumes") or []] + [x.get("volumes") or [] for x in (e.get("versions") or [])]
                       for v in vs]
                _ds = [d for d in _ds if d[:4].isdigit()]
                return min(_ds) if _ds else ""
            _fd = [_first_date(e) for e in _grp]
            for _cut in (4, 7):                      # 年 → 年月 の順に細かくして区別を試みる
                _sfx = [d[:_cut] for d in _fd]
                if all(_sfx) and len(set(_sfx)) == len(_sfx):
                    for _ce, _x in zip(_grp, _sfx):
                        _ce["label"] = f"{_base}({_x})"
                    break
            else:
                for _i, (_ce, _d) in enumerate(zip(_grp, _fd), 1):
                    _ce["label"] = f"{_base}({_d[:4]})[{_i}]" if _d else f"{_base}[{_i}]"
    ...
    if _pp_dir.is_dir():
        ...
        for _f in sorted(_pp_dir.glob("*.yml")):
            ...
            if _pdrv - set(_pd.get("genres") or []):
                ...
                _touched = True
            # ★年レンジの追随用に「override 適用前」の巻年を控える(下で使う)。
            def _pp_years(_page):
                _ys = set()
                for _e2 in _page.get("editions") or []:
                    for _pool2 in [_e2.get("volumes") or []] + \
                            [vv.get("volumes") or [] for vv in (_e2.get("versions") or [])]:
                        for _v2 in _pool2:
                            _d2 = str((_v2 or {}).get("release_date") or "")
                            if len(_d2) >= 4 and _d2[:4].isdigit():
                                _ys.add(int(_d2[:4]))
                return _ys
            _years_before = _pp_years(_pd)
                    ...
                    for _v in _pool:
                        ...
                        if _ck3 in _co:
                            ...
                            if _cu:
                                _v["cover_url"] = _cu
                                _touched = True
                        # ★発売日も本流(_eff_date)と同じ規則でここに通す(2026-09-04 ユーザ発見・実害)。
                        #   予約巻は **後から発売日が動く**(延期/前倒し)のに、予約頁は本流を通らないため
                        #   release-date-override.jsonl に正しく書いても**永久に届かなかった**
                        #   = 「予約頁は本流を通らない」穴の5件目(magazine/genre/genre-append/cover に続く)。
                        #   実例: すてごろブッチ!(9784091543295) 2026-08-28 のまま → 実際は 2026-09-28(1か月延期)。
                        #   規則は cover-override と同型「キーが在れば必ず勝つ」。補完(supplement)は空の時だけ。
                        _rdo_p = get_release_date_override()
                        if _ck3 in _rdo_p:
                            if _v.get("release_date") != _rdo_p[_ck3]:
                                _v["release_date"] = _rdo_p[_ck3]
                                _touched = True
                        elif not _v.get("release_date"):
                            _rd_s = get_release_date_supplement().get(_ck3)
                            if _rd_s:
            ...
            #   ★安全弁: 現在の年が「巻から導かれた値」と一致する時だけ追随する
            #   (手で入れた年・連載開始年を上書きしない)。
            _years_after = _pp_years(_pd)
            if _years_before and _years_after and _years_after != _years_before:
                if _pd.get("year_started") == min(_years_before):
                    if _pd["year_started"] != min(_years_after):
                        _pd["year_started"] = min(_years_after)
                        _touched = True
                if _pd.get("year_ended") == max(_years_before):
                    if _pd["year_ended"] != max(_years_after):
                        _pd["year_ended"] = max(_years_after)
                        _touched = True
