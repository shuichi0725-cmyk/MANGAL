...
const slice = cand.slice(9000, 10202).map((r,i) => ({
  rank: 9001 + i, key: r.series_qid ? `${r.series_qid}|${r.title}` : (r.author_qid ? `${r.author_qid}|${r.title}` : `sid:${r.id}|${r.title}`),
...
fs.writeFileSync(".cache/next-1202.json", JSON.stringify(slice, null, 2));
