...
const slice = cand.slice(12202, 20202).map((r,i) => ({
  rank: 12203 + i, key: r.series_qid ? `${r.series_qid}|${r.title}` : (r.author_qid ? `${r.author_qid}|${r.title}` : `sid:${r.id}|${r.title}`),
...
fs.writeFileSync(".cache/next-8000.json", JSON.stringify(slice, null, 2));
