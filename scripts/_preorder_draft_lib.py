...
_VOL_TAIL = re.compile(
    ...
    r"|[\s　]*[（(]?\s*[巻其][ノの之]\s*(?:\d{1,3}|[一二三四五六七八九十]{1,4})\s*(?:[（(][^()（）]*[)）])?\s*[)）]?"  # ★巻ノ1/巻之五/其ノ2(2026-09-24)
    r"|[\s　]*[（(]\s*第?\s*\d{1,3}\s*集\s*(?:[（(][^()（）]*[)）])?\s*[)）]?"            # ★(第2集)/(12集)
    r"|[\s　]+第\s*(?:\d{1,3}|[一二三四五六七八九十]{1,4})\s*集"                         # ★ 第3集
    r"|[\s　]*[（(]\s*v\.\s*\d{1,3}\s*[)）]"                                             # ★(v.2)
    r"|[\s　]*[（(]\s*第?\s*\d{1,3}\s*巻?\s*[（(][^()（）]*[)）]\s*[)）]"                 # ★(2(副題))
...
def author_is_anthology(rec):
    """★アンソロジー誌/合同誌(= 掲載対象外。CLAUDE.md 関連書patterns「アンソロジー」)。
    楽天は複数作家の合同本の著者欄に **文字どおり「アンソロジー」** を入れる。
    2026-09-20 ユーザ指摘(ねこぱんち)を起点に掃引したところ、この signal で本番に
    **14頁**すり抜けていた(思い出食堂/ひとりごはん/ときめきごはん/みんなの食卓/アウトドアごはん/
    世にも奇妙な時代劇/まんがでイッキ読み/ねこぱんち 各号)。全14頁とも予約経路由来=ここが唯一の漏れ口。
    ★実測(2026-09-20 harvest 3,674件): 該当25件は**全部アンソロジー**(偽陽性0)。
    題に「アンソロジー」が無い号(思い出食堂 豆腐の味噌汁編 等)も捕まるので題ゲートでは代替できない。
    ★完全一致で見ること: 「アンソロジー編集部」等の部分一致まで拾うと作家名を巻き込みうる。"""
    return any(x.strip() == "アンソロジー" for x in str(rec.get("author") or "").split("/"))
...
_VOL_KANA = re.compile(r"(?:ダイ)?[イチニサンヨンゴロクナナハチキュウジュウゼロ第]+カン\s*$"
                       r"|[\s　]*(?:マキノ|カンノ)[イチニサンヨンゴロクナナハチキュウジュウゼロ]+\s*$"  # ★巻之五=マキノゴ/カンノサン(2026-09-24 楽天ヨミ実測)。★ソノN は剥がさない(題側の「その六」は巻として剥がしていない=題とヨミがずれる)
...
def clean_kana(kana, subtitle=None, base=None):
    ...
    for _ in range(3):
        ...
        if k2 == k:
            break
        k = k2
    # ★副題を差し引いた後に残る前後の波ダッシュ/チルダを落とす(2026-09-19 実踏:
    #   「ツガイシッカク~」「オットガシニカケタノデ…ハナシ~」が preview まで通った)。
    #   楽天ヨミは副題を ~…~ で括るため、差し引き後に開き側の ~ が語尾に残る。
    #   ★長音符 ー は落とさない(コーヒー型を壊す)。内側の ~ も触らない(範囲記号のことがある)。
    k = re.sub(r"^[〜～~]+|[〜～~]+$", "", k)
