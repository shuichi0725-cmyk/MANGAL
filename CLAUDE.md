...
後退蒸留=backward-distill / 月次蒸留=monthly-distill / 作品名+リンク=percase-fix / 新規追加=new-manga-register / 巻抜け仮想=volgap-audit / **差分反映して=diff-deploy(データのみ本番へ~15分・コード変更はabort→週次)** / **機能蒸留して=feature-distill(コードのみ本番へ~30分・非漫画面+チャンクだけPUT・漫画頁/索引不変)** / **Wiki蒸留して=wiki-distill(Wikipedia書誌で長期連載復元)** / **本番化して=productionize-drafts(確認済み予約ドラフトをpreorder-pages恒久化→週次で本番公開・preview解放)**。
